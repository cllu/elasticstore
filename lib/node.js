...
function Node(data) {
  ...
  _.forOwn(this._schema, function (value, key) {
    ...
    // convert data to special type, like Date to Moment object
    if (value.type && value.type.name == 'SchemaTypeMoment' && this[key] && !moment.isMoment(this[key])) {
 ...
 *
 * @param data
 * @returns {Node} an instance of Node
 */
...
Node.updateById = function (id, update) {
  ...
  return self.findById(id).then(function (node) {
...
Node.prototype.getContext = function () {
  return this.constructor.getContext();
};
